...
export let executeCell = (cell: md.Cell): ChildProcessWithoutNullStreams => {
    const javascriptCell = new Cell(cell, workspace.getConfiguration('codebook-md.javascript'));
    return javascriptCell.execute();
};
...
export class Cell {
    ...
    constructor(cell: md.Cell, javascriptConfig: vscode.WorkspaceConfiguration | undefined) {
        this.config = new Config(javascriptConfig);
