...
- **VS Code Terminal Environment:**
  - All executable languages now inherit VS Code integrated terminal environment variables automatically (shell, Go, JavaScript/TypeScript, Python, SQL, HTTP)
  - Settings from `terminal.integrated.env.*` override process variables for consistent runs

### Added

- **Workspace Token Substitutions:**
  - Environment values support `${workspaceFolder}`, `${workspaceFolderBasename}`, and `${pathSeparator}` placeholders when merging terminal settings
  - Enables portable paths without hardcoding directories across platforms
